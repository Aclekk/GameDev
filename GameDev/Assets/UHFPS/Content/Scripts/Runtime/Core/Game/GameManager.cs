...
namespace UHFPS.Runtime
{
    ...
    public class GameManager : Singleton<GameManager>
    {
        ...
        private void Update()
        {
            if (isPointerShown && pointerInteractAction != null)
            {
                Vector2 pointerDelta = InputManager.ReadInput<Vector2>(Controls.POINTER_DELTA);
                Image pointerImage = PointerImage;
                ...
                if (GameTools.Raycast(pointerRay, out RaycastHit hit, 5, pointerCullLayers, pointerInteractLayer) && hit.collider.TryGetComponent(out IInteractStart interactStart))
                {
                    pointerImage.sprite = HoverPointer;
                    pointerImage.rectTransform.sizeDelta = HoverPointerSize;
                ...
                else
                {
                    ...
                    pointerImage.rectTransform.sizeDelta = NormalPointerSize;
                }
            }
